...
BUTTON_EMOJI_IDS = {
    ...
    "admin_summary": "5391316230118321870",
...
CREATE TABLE IF NOT EXISTS summary_logs(id INTEGER PRIMARY KEY AUTOINCREMENT,summary_date TEXT UNIQUE,created_at TEXT);
...
def _summary_metrics(day):
    """Return calendar-day metrics using Bangladesh time boundaries."""
    start_local = datetime(day.year, day.month, day.day, tzinfo=BD_TZ)
    end_local = start_local + timedelta(days=1)
    start = _utc_text(start_local)
    end = _utc_text(end_local)
    y_start = _utc_text(start_local - timedelta(days=1))
    y_end = start
    d7_start = _utc_text(start_local - timedelta(days=6))
    d30_start = _utc_text(start_local - timedelta(days=29))

    total_users = c.execute("SELECT COUNT(*) FROM users").fetchone()[0]
    active_users = c.execute("SELECT COUNT(*) FROM users WHERE blocked=0").fetchone()[0]
    blocked_users = c.execute("SELECT COUNT(*) FROM users WHERE blocked=1").fetchone()[0]
    new_today = c.execute("SELECT COUNT(*) FROM users WHERE created_at>=? AND created_at<?", (start,end)).fetchone()[0]
    new_yesterday = c.execute("SELECT COUNT(*) FROM users WHERE created_at>=? AND created_at<?", (y_start,y_end)).fetchone()[0]
    new_7d = c.execute("SELECT COUNT(*) FROM users WHERE created_at>=? AND created_at<?", (d7_start,end)).fetchone()[0]
    new_30d = c.execute("SELECT COUNT(*) FROM users WHERE created_at>=? AND created_at<?", (d30_start,end)).fetchone()[0]

    reward_today = c.execute("SELECT COALESCE(SUM(today_income),0) FROM users WHERE today_date=?", (day.isoformat(),)).fetchone()[0]
    # Referral earnings are part of user income only when actually credited.
    referral_today = c.execute("SELECT COALESCE(SUM(amount),0) FROM referral_commissions WHERE created_at>=? AND created_at<?", (start,end)).fetchone()[0]
    submissions_today = c.execute("SELECT COUNT(*), COALESCE(SUM(rows_count),0), COALESCE(SUM(estimated_profit),0) FROM submissions WHERE created_at>=? AND created_at<?", (start,end)).fetchone()
    withdrawals_today = c.execute("SELECT COUNT(*), COALESCE(SUM(amount),0) FROM withdrawals WHERE created_at>=? AND created_at<?", (start,end)).fetchone()
    approved_w = c.execute("SELECT COUNT(*), COALESCE(SUM(amount),0) FROM withdrawals WHERE status='approved' AND created_at>=? AND created_at<?", (start,end)).fetchone()
    pending_w = c.execute("SELECT COUNT(*), COALESCE(SUM(amount),0) FROM withdrawals WHERE status='pending' AND created_at>=? AND created_at<?", (start,end)).fetchone()
    rejected_w = c.execute("SELECT COUNT(*), COALESCE(SUM(amount),0) FROM withdrawals WHERE status='rejected' AND created_at>=? AND created_at<?", (start,end)).fetchone()
    reports_today = c.execute("SELECT COUNT(*), COALESCE(SUM(green_count),0), COALESCE(SUM(processed_users),0), COALESCE(SUM(total_reward),0) FROM reports WHERE uploaded_at>=? AND uploaded_at<?", (start_local.timestamp(), end_local.timestamp())).fetchone()
    total_balance = c.execute("SELECT COALESCE(SUM(balance),0) FROM users").fetchone()[0]
    total_referrals = c.execute("SELECT COUNT(*) FROM users WHERE referrer_id IS NOT NULL").fetchone()[0]
    referral_today_count = c.execute("SELECT COUNT(*) FROM users WHERE referrer_id IS NOT NULL AND created_at>=? AND created_at<?", (start,end)).fetchone()[0]

    def pct(a,b):
        return ((a-b)/b*100.0) if b else (100.0 if a else 0.0)
    return dict(total_users=total_users,active_users=active_users,blocked_users=blocked_users,
                new_today=new_today,new_yesterday=new_yesterday,new_7d=new_7d,new_30d=new_30d,
                growth_day=pct(new_today,new_yesterday), reward_today=float(reward_today or 0),
                referral_today=float(referral_today or 0), submissions=int(submissions_today[0] or 0),
                submitted_rows=int(submissions_today[1] or 0), submission_profit=float(submissions_today[2] or 0),
                withdrawals=int(withdrawals_today[0] or 0), withdrawal_amount=float(withdrawals_today[1] or 0),
                approved_w=int(approved_w[0] or 0), approved_amount=float(approved_w[1] or 0),
                pending_w=int(pending_w[0] or 0), pending_amount=float(pending_w[1] or 0),
                rejected_w=int(rejected_w[0] or 0), rejected_amount=float(rejected_w[1] or 0),
                reports=int(reports_today[0] or 0), green_count=int(reports_today[1] or 0),
                processed_users=int(reports_today[2] or 0), report_reward=float(reports_today[3] or 0),
                total_balance=float(total_balance or 0), total_referrals=total_referrals,
                referral_today_count=referral_today_count)

def _summary_text(day, m, automatic=False):
    title = "🌙 24 HOURS BOT SUMMARY" if automatic else "📊 TODAY'S BOT SUMMARY"
    growth_arrow = "📈" if m["growth_day"] >= 0 else "📉"
    return (
        "╔══════════════════════════╗\n"
        f"   {title}\n"
        "╚══════════════════════════╝\n\n"
        f"📅 Date: <b>{day.strftime('%d %B %Y')}</b>\n\n"
        "👥 <b>USER GROWTH</b>\n"
        f"├ Total Users: <b>{m['total_users']}</b>\n"
        f"├ Active: <b>{m['active_users']}</b>  •  Blocked: <b>{m['blocked_users']}</b>\n"
        f"├ New Today: <b>+{m['new_today']}</b>\n"
        f"├ Yesterday: <b>{m['new_yesterday']}</b>\n"
        f"├ Last 7 Days: <b>+{m['new_7d']}</b>\n"
        f"└ Last 30 Days: <b>+{m['new_30d']}</b>\n\n"
        f"{growth_arrow} <b>GROWTH</b>: <b>{m['growth_day']:+.1f}%</b> vs yesterday\n\n"
        "💰 <b>EARNING / ACTIVITY</b>\n"
        f"├ User Income Today: <b>৳{m['reward_today']:,.2f}</b>\n"
        f"├ Referral Commission: <b>৳{m['referral_today']:,.2f}</b>\n"
        f"├ Today Submissions: <b>{m['submissions']}</b>\n"
        f"├ Submitted Rows: <b>{m['submitted_rows']}</b>\n"
        f"└ Estimated Reward: <b>৳{m['submission_profit']:,.2f}</b>\n\n"
        "📤 <b>WITHDRAWAL</b>\n"
        f"├ Requests: <b>{m['withdrawals']}</b> • ৳{m['withdrawal_amount']:,.2f}\n"
        f"├ Approved: <b>{m['approved_w']}</b> • ৳{m['approved_amount']:,.2f}\n"
        f"├ Pending: <b>{m['pending_w']}</b> • ৳{m['pending_amount']:,.2f}\n"
        f"└ Rejected: <b>{m['rejected_w']}</b> • ৳{m['rejected_amount']:,.2f}\n\n"
        "📁 <b>REPORT / FILE ACTIVITY</b>\n"
        f"├ Reports: <b>{m['reports']}</b>\n"
        f"├ Green UID: <b>{m['green_count']}</b>\n"
        f"├ Processed Users: <b>{m['processed_users']}</b>\n"
        f"└ Report Reward: <b>৳{m['report_reward']:,.2f}</b>\n\n"
        "🔗 <b>REFERRAL</b>\n"
        f"├ New Referrals Today: <b>{m['referral_today_count']}</b>\n"
        f"└ Total Referred Users: <b>{m['total_referrals']}</b>\n\n"
        "💳 <b>CURRENT BALANCE</b>\n"
        f"└ Total User Balance: <b>৳{m['total_balance']:,.2f}</b>\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🤖 <i>Bot Growth & Performance Report</i>"
    )

def console_admin_kb():
    report_on = setting("console_report", "0") == "1"
    payment_on = setting("console_payment", "0") == "1"
    return ReplyKeyboardMarkup(
        keyboard=[
            [kbbtn(text="Console Report ON"), kbbtn(text="Console Report OFF")],
            [kbbtn(text="Console Payment ON"), kbbtn(text="Console Payment OFF")],
            [kbbtn(text="Back to Admin Panel")],
        ],
        resize_keyboard=True,
    )

async def console_admin(m):
    if not m or not m.from_user or m.from_user.id != ADMIN_ID:
        return
    t = clean_button_text(m.text)
    if t == "Console":
        ro = "ON 🟢" if setting("console_report", "0") == "1" else "OFF 🔴"
        po = "ON 🟢" if setting("console_payment", "0") == "1" else "OFF 🔴"
        return await ans(
            m,
            "⚙️ Console\n\n"
            f"📊 Console Report: <b>{ro}</b>\n"
            f"💳 Console Payment: <b>{po}</b>\n\n"
            "ON করলে channel-এ automated DEMO activity message যাবে।\n"
            "প্রতিটি generated message-এ স্পষ্ট <b>DEMO</b> label থাকবে।",
            reply_markup=console_admin_kb()
        )
    if t == "Console Report ON":
        save_setting("console_report", "1")
        return await ans(m, "📊 Console Report <b>ON</b> করা হয়েছে।\n\nReport Channel configured থাকলে automated DEMO activity যাবে।", reply_markup=console_admin_kb())
    if t == "Console Report OFF":
        save_setting("console_report", "0")
        return await ans(m, "📊 Console Report <b>OFF</b> করা হয়েছে।", reply_markup=console_admin_kb())
    if t == "Console Payment ON":
        save_setting("console_payment", "1")
        return await ans(m, "💳 Console Payment <b>ON</b> করা হয়েছে।\n\nPayment Channel configured থাকলে automated DEMO withdrawal activity যাবে।", reply_markup=console_admin_kb())
    if t == "Console Payment OFF":
        save_setting("console_payment", "0")
        return await ans(m, "💳 Console Payment <b>OFF</b> করা হয়েছে।", reply_markup=console_admin_kb())
    if t == "Back to Admin Panel":
        state.pop(ADMIN_ID, None)
        return await ans(m, "Admin Panel", reply_markup=am())

async def _console_demo_report(bot):
    target = setting("report_channel").strip()
    if not target:
        return False
    count = random.randint(2, 25)
    reward = random.randint(50, 500)
    category = random.choice(["Report", "Account Report", "Daily Report", "Work Report"])
    text = (
        "🧪 <b>DEMO</b>\n"
        "📊 <b>REPORT SUCCESSFUL</b>\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        f"📁 Category: <b>{category}</b>\n"
        f"🟢 Green/Okay UID: <b>{count}</b>\n"
        f"👥 Processed Users: <b>{random.randint(1, max(1, count))}</b>\n"
        f"💰 Total Reward: <b>৳{reward:,.2f}</b>\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "🤖 Console Demo Activity\n"
        "⚠️ This is a demo activity message."
    )
    try:
        await sendmsg(bot, target, text)
        return True
    except Exception as e:
        print(f"[console-report] {e}")
        return False

async def _console_demo_payment(bot):
    target = setting("payment_approve_channel").strip()
    if not target:
        return False
    amount = random.randint(100, 3000)
    fee = round(amount * random.choice([0.0, 0.01, 0.02]), 2)
    method = random.choice(["Bkash", "Nagad"])
    demo_id = random.randint(10000, 99999)
    text = (
        "🧪 <b>DEMO</b>\n"
        "💳 <b>WITHDRAWAL SUCCESSFUL</b>\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        f"💰 Amount Paid: <b>৳{amount:,.2f}</b>\n"
        f"Withdrawal Fee: <b>৳{fee:,.2f}</b>\n"
        f"📱 Method: <b>{method}</b>\n"
        f"🔖 Request ID: <b>#DEMO{demo_id}</b>\n"
        "━━━━━━━━━━━━━━━━━━━\n"
        "✅ Withdrawal Completed\n"
        "Console demo activity message.\n"
        "⚠️ This is not a real payment."
    )
    try:
        await sendmsg(bot, target, text)
        return True
    except Exception as e:
        print(f"[console-payment] {e}")
        return False

async def console_loop(bot):
    """Generate clearly labelled DEMO activity while Console switches are ON."""
    while True:
        try:
            report_on = setting("console_report", "0") == "1"
            payment_on = setting("console_payment", "0") == "1"
            did_work = False
            if report_on:
                did_work = await _console_demo_report(bot) or did_work
            if payment_on:
                did_work = await _console_demo_payment(bot) or did_work
            # Keep activity irregular rather than posting at a fixed cadence.
            await asyncio.sleep(random.randint(180, 600) if did_work else 30)
        except Exception as e:
            print(f"[console-loop] {type(e).__name__}: {e}")
            await asyncio.sleep(30)

async def summary_admin(m):
    """Reliable admin Summary button handler. Uses a plain Telegram reply path so
    custom-emoji/entity formatting cannot make the button appear unresponsive."""
    if not m or not m.from_user or m.from_user.id != ADMIN_ID:
        return
    day = _bd_now().date()
    try:
        metrics = _summary_metrics(day)
        text = _summary_text(day, metrics)
        # _summary_text uses HTML tags; remove them for this fail-safe path.
        text = re.sub(r"<[^>]*>", "", text)
        return await m.answer(text, reply_markup=am())
    except Exception as e:
        print(f"[summary] {type(e).__name__}: {e}")
        try:
            return await m.answer(
                "❌ Summary তৈরি করতে সমস্যা হয়েছে।\n\n"
                "Database-এর required data পাওয়া যাচ্ছে না বা query error হয়েছে।",
                reply_markup=am()
            )
        except Exception as e2:
            print(f"[summary-fallback] {type(e2).__name__}: {e2}")


async def daily_summary_loop(bot):
    """Send one Bangladesh-time calendar-day summary after every midnight."""
    while True:
        try:
            now_local = _bd_now()
            next_midnight = datetime(now_local.year, now_local.month, now_local.day, tzinfo=BD_TZ) + timedelta(days=1)
            await asyncio.sleep(max(1, (next_midnight - now_local).total_seconds() + 2))
            day = (next_midnight - timedelta(days=1)).date()
            day_key = day.isoformat()
            # Duplicate protection survives bot restarts/database restores.
            if not c.execute("SELECT 1 FROM summary_logs WHERE summary_date=?", (day_key,)).fetchone():
                metrics = _summary_metrics(day)
                await bot.send_message(ADMIN_ID, _summary_text(day, metrics, automatic=True))
                c.execute("INSERT OR IGNORE INTO summary_logs(summary_date,created_at) VALUES(?,?)", (day_key, now()))
                db.commit()
...
async def admin(m):
    ...
    if t in {"Console", "Console Report ON", "Console Report OFF", "Console Payment ON", "Console Payment OFF"}:
        return await console_admin(m)
    if t in ("Summary", "📊 Summary"):
        return await summary_admin(m)
    if t=="Refer Management":
...
dp.message.register(console_admin, F.from_user.id == ADMIN_ID, lambda m: clean_button_text(m.text) in {"Console", "Console Report ON", "Console Report OFF", "Console Payment ON", "Console Payment OFF", "Back to Admin Panel"})
dp.message.register(
    summary_admin,
    F.from_user.id == ADMIN_ID,
    lambda m: clean_button_text(m.text) in {"Summary", "📊 Summary"}
)
...
async def main():
    ...
    asyncio.create_task(daily_summary_loop(b))
    asyncio.create_task(console_loop(b))
