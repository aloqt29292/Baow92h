...
async def withdraw(m):
    ...
    if not rows:
        return await ans(m, "বর্তমানে কোনো withdrawal method active নেই.",reply_markup=um(m.from_user.id))
    state[m.from_user.id]={"wcat":True}
    buttons=[]
    for x in rows:
        n=str(x["name"]).strip()
        low=n.lower()
        icon=PE.get("bkash") if ("bkash" in low or "বিকাশ" in low) else (PE.get("nagad") if ("nagad" in low or "nogod" in low or "নগদ" in low) else None)
        buttons.append([ibtn(text=n, callback_data=f"wcat:{x['id']}", icon_custom_emoji_id=icon)])
    buttons.append([ibtn(text="Cancel", callback_data="wcancel", icon_custom_emoji_id=PE.get("cross"))])
    await ans(m, "💳 Withdrawal method নির্বাচন করুন:", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def withdrawal_category_selection(m):
    # Kept for compatibility with older text-based flow; current flow uses inline callbacks.
    if not await ensure_joined(m): return
    return await ans(m, "দয়া করে Withdrawal-এর inline button থেকে একটি method নির্বাচন করুন।")

async def withdrawal_method_callback(q):
    if not await ensure_joined(q.message): return
    try:
        wid=int(q.data.split(":",1)[1])
    except Exception:
        return await q.answer("Invalid method.", show_alert=True)
    x=c.execute("SELECT * FROM withdrawal_categories WHERE id=? AND active=1",(wid,)).fetchone()
    if not x:
        return await q.answer("এই withdrawal method বর্তমানে চালু নেই।", show_alert=True)
    nml=str(x["name"]).strip().lower()
    if "বিকাশ" in nml or "bkash" in nml:
        wallet_label="বিকাশ"
    elif "নগদ" in nml or "nagad" in nml or "nogod" in nml:
        wallet_label="নগদ"
    else:
        wallet_label=str(x["name"]).strip()
    state[q.from_user.id]={
        "wid":x["id"], "wn":x["name"],
        "min":float(x["minimum"] or 0), "max":float(x["maximum"] or 0),
        "fee":float(x["fee"] or 0), "wallet_label":wallet_label,
        "withdraw_step":"wallet"
    }
    await q.answer()
    try:
        await q.message.edit_text(f"📱 আপনার {wallet_label} নাম্বার দিন।")
    except Exception:
        await ans(q.message, f"📱 আপনার {wallet_label} নাম্বার দিন।")

async def withdrawal_confirm_callback(q):
    uid=q.from_user.id
    s=state.get(uid,{})
    if not s.get("withdraw_confirm"):
        return await q.answer("এই withdrawal session আর active নেই।", show_alert=True)
    if q.data == "wcancel":
        state.pop(uid,None)
        await q.answer("Cancelled")
        try: await q.message.edit_text("❌ Withdrawal cancelled.", reply_markup=None)
        except Exception: pass
        return await q.message.answer("Main Menu", reply_markup=um(uid))

    u=c.execute("SELECT balance FROM users WHERE user_id=?",(uid,)).fetchone()
    balance=float(u["balance"] or 0) if u else 0
    total=float(s.get("total_debited",s["requested_amount"]))
    if balance < total:
        state.pop(uid,None)
        await q.answer("Balance insufficient.", show_alert=True)
        try: await q.message.edit_text("❌ আপনার balance আর পর্যাপ্ত নেই। Withdrawal বাতিল করা হয়েছে।")
        except Exception: pass
        return await q.message.answer("Main Menu", reply_markup=um(uid))

    c.execute("UPDATE users SET balance=balance-? WHERE user_id=?",(total,uid))
    full_name=" ".join(x for x in [q.from_user.first_name or "", q.from_user.last_name or ""] if x).strip()
    c.execute("INSERT INTO withdrawals(user_id,username,full_name,category_id,category_name,amount,wallet,fee,total_debited,status,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(
        uid,q.from_user.username or "",full_name,s["wid"],s["wn"],s["value"],s["wallet"],s.get("fee",0),total,"pending",now()))
    wid=c.lastrowid
    db.commit(); state.pop(uid,None)
    await q.answer("Withdrawal request submitted.")
    try:
        await q.message.edit_text(
            f"✅ Withdrawal request জমা হয়েছে.\n\nRequest ID: #{wid}\nMethod: {s['wallet_label']}\nAmount: {s['requested_amount']:.2f} টাকা\nFee: {s.get('fee',0):.2f} টাকা\nআপনি পাবেন: {s['value']:.2f} টাকা\n\nAdmin approval-এর অপেক্ষায়."
        )
    except Exception: pass
    await q.message.answer("Main Menu", reply_markup=um(uid))
    try:
        await sendmsg(q.message.bot, ADMIN_ID,
            f"New Withdrawal Request\n\nID: {wid}\nUsername: @{q.from_user.username or 'N/A'}\nUser ID: {uid}\nCategory: {s['wn']}\nAmount to Pay: {s['value']:.2f} টাকা\nFee: {s.get('fee',0):.2f} টাকা\nTotal Debited from User: {total:.2f} টাকা\nWallet: {s['wallet']}\nTime: {now()}",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[ibtn("Approve", callback_data=f"wapprove:{wid}", icon_custom_emoji_id=PE["done"]), ibtn("Reject + Refund", callback_data=f"wreject:{wid}", icon_custom_emoji_id=PE["cross"])]]))
    except Exception as e:
        print(f"[withdrawal-admin-notify] {wid}: {e}")
...
async def user_state(m):
    ...
    if s.get("withdraw_step") == "wallet":
        wallet=m.text.strip()
        if not wallet:
            return await ans(m, "❌ সঠিক wallet number দিন।")
        if len(wallet) < 8 or len(wallet) > 20:
            return await ans(m, "❌ সঠিক mobile number দিন।")
        s["wallet"]=wallet
        s["withdraw_step"]="amount"
        state[m.from_user.id]=s
        return await ans(m, "💰 কত টাকা Withdraw করতে চান? শুধু amount লিখুন।")
    ...
    if s.get("withdraw_step") == "amount":
        ...
        if a < s["min"]:
            return await ans(m, f"❌ Minimum withdrawal: {s['min']:.2f} টাকা")
        if s["max"] > 0 and a > s["max"]:
            return await ans(m, f"❌ Maximum withdrawal: {s['max']:.2f} টাকা")
        fee=float(s.get("fee",0) or 0)
        if fee >= a:
            return await ans(m, f"❌ Withdrawal fee ({fee:.2f}) amount-এর চেয়ে কম হতে হবে।")
        u=c.execute("SELECT balance FROM users WHERE user_id=?",(m.from_user.id,)).fetchone()
        balance=float(u["balance"] or 0) if u else 0
        if a > balance:
            return await ans(m, f"❌ Balance যথেষ্ট নয়.\nআপনার Balance: {balance:.2f} টাকা")
        payout=a-fee
        s["requested_amount"]=a; s["value"]=payout; s["fee"]=fee; s["total_debited"]=a; s["withdraw_confirm"]=True
        s["withdraw_step"]="confirm"
        state[m.from_user.id]=s
        text=(f"💸 Withdrawal Confirmation\n\n"
              f"Method: {s['wallet_label']}\n"
              f"Number: {s['wallet']}\n"
              f"Amount: {a:.2f} টাকা\n"
              f"Fee: {fee:.2f} টাকা\n"
              f"You will receive: {payout:.2f} টাকা\n\n"
              "সব তথ্য ঠিক থাকলে Confirm চাপুন।")
        kb=InlineKeyboardMarkup(inline_keyboard=[[ibtn("Confirm", callback_data="wconfirm", icon_custom_emoji_id=PE["done"]), ibtn("Cancel", callback_data="wcancel", icon_custom_emoji_id=PE["cross"]) ]])
...
dp.callback_query.register(withdrawal_method_callback,F.data.startswith("wcat:"))
dp.callback_query.register(withdrawal_confirm_callback,F.data.in_({"wconfirm","wcancel"}))
