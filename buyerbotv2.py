...
async def summary_admin(m):
    ...
    try:
        ...
        return await ans(m, _summary_text(day, metrics), reply_markup=am())
    except Exception as e:
        print(f"[summary] {e}")
...
dp.message.register(summary_admin,F.from_user.id==ADMIN_ID,lambda m: clean_button_text(m.text) in {"Summary", "📊 Summary"})
