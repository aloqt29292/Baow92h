...
def ibtn(text, **kwargs):
    ...
    try:
        ...
        kwargs.pop("style", None)
        return _InlineKeyboardButton(text=text, **kwargs)
TOKEN = "8856510779:AAGI2t9O9MmWZLoXrm2JBFv2P5-VkREELn8"
ADMIN_ID = 6995426618
    ...
    for user_id, submissions in user_files.items():
        ...
        for s in submissions:
            for uid in submission_uids.get(s["id"],[]):
                ...
                # Pay referral commission per Green UID. Admin-designated referrers
                # use only their category-specific admin rule; normal users use default.
                ref_row=c.execute("SELECT referrer_id FROM referrals WHERE referred_id=?", (user_id,)).fetchone()
                if ref_row:
                    try:
                        referral_commission_for_green(ref_row["referrer_id"], user_id, int(s["category_id"] or 0), s["category_name"] or category_name, uid, amount)
                    except Exception as e:
                        print(f"referral commission error for {uid}: {e}")
                ok_count += 1
                reward += amount
...
def um(uid):
    ...
    k=[
        ...
        [kbbtn(text="Refer"), kbbtn(text="Support")]
    ]
...
def referral_default_settings():
    return setting("ref_user_commission_enabled", "1") == "1", setting("ref_default_commission_type", "fixed"), float(setting("ref_default_commission_value", "0.05") or 0.05)

def referral_link_from_username(bot_username, uid):
    return f"https://t.me/{bot_username}?start=ref_{uid}"

def referral_is_admin(referrer_id):
    return c.execute("SELECT 1 FROM admin_commissions WHERE admin_id=? LIMIT 1", (referrer_id,)).fetchone() is not None

def referral_admin_rule(referrer_id, category_id):
    return c.execute("SELECT * FROM admin_commissions WHERE admin_id=? AND category_id=?", (referrer_id, category_id)).fetchone()

def referral_commission_for_green(referrer_id, referred_id, category_id, category_name, uid, base_amount):
    """Return and persist one commission for one Green UID.
    Admin-designated referrers NEVER receive the normal user/default rate.
    Their only possible commission is the explicitly configured category rule.
    """
    if not referrer_id or referrer_id == referred_id:
        return 0.0
    if c.execute("SELECT 1 FROM referral_commissions WHERE referrer_id=? AND referred_id=? AND category_id=? AND uid=?", (referrer_id,referred_id,category_id,uid)).fetchone():
        return 0.0

    is_admin = referral_is_admin(referrer_id)
    ctype = None
    rate = 0.0
    if is_admin:
        if setting("ref_admin_commission_enabled", "1") != "1":
            return 0.0
        rule = referral_admin_rule(referrer_id, category_id)
        if not rule or not rule["enabled"]:
            return 0.0
        ctype, rate = rule["commission_type"], float(rule["value"] or 0)
    else:
        enabled, ctype, rate = referral_default_settings()
        if not enabled:
            return 0.0

    if ctype == "percent":
        commission = max(0.0, float(base_amount or 0) * rate / 100.0)
    else:
        commission = max(0.0, rate)
    if commission <= 0:
        return 0.0

    c.execute("INSERT INTO referral_commissions(referrer_id,referred_id,category_id,category_name,uid,base_amount,commission,commission_type,rate,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)", (referrer_id,referred_id,category_id,category_name or "",str(uid),float(base_amount or 0),commission,ctype,rate,now()))
...
async def refer_section(m):
    ...
    try:
        ...
        if not username:
            return await ans(m, "Referral link তৈরি করা যাচ্ছে না। Bot username সেট নেই।", reply_markup=um(m.from_user.id))
        link = referral_link_from_username(username, m.from_user.id)
        count = c.execute("SELECT COUNT(*) AS n FROM referrals WHERE referrer_id=?", (m.from_user.id,)).fetchone()["n"]
        earned = c.execute("SELECT COALESCE(SUM(commission),0) AS n FROM referral_commissions WHERE referrer_id=?", (m.from_user.id,)).fetchone()["n"]
        text = ("Refer & Earn\n\n"
                "আপনার Referral Link শেয়ার করুন। আপনার Referral থেকে আসা প্রতিটি Green ID-এর জন্য প্রযোজ্য কমিশন আপনার Balance-এ যোগ হবে।\n\n"
                f"Default commission: 0.05 BDT / Green ID\n"
                f"Total referrals: {count}\n"
                f"Referral earnings: {float(earned or 0):.2f} BDT\n\n"
                "নিচের Copy Referral Link বাটনে চাপ দিলে আপনার Referral Link সরাসরি কপি হবে।")
        if CopyTextButton is not None:
            kb=InlineKeyboardMarkup(inline_keyboard=[[ _InlineKeyboardButton(text="Copy Referral Link", copy_text=CopyTextButton(text=link)) ]])
        else:
            kb=InlineKeyboardMarkup(inline_keyboard=[[ _InlineKeyboardButton(text="Referral Link", url=link) ]])
        return await ans(m, text, reply_markup=kb)
    except Exception as e:
        print(f"referral section error: {e}")
        return await ans(m, "Referral system খুলতে সমস্যা হয়েছে।", reply_markup=um(m.from_user.id))


def refer_admin_kb():
    return ReplyKeyboardMarkup(keyboard=[
        [kbbtn(text="Refer Statistics")],
        [kbbtn(text="Enable/Disable User Commission")],
        [kbbtn(text="Enable/Disable Admin Commission")],
        [kbbtn(text="Set Default Commission")],
        [kbbtn(text="Set Admin Green Commission")],
        [kbbtn(text="List Admin Commission")],
        [kbbtn(text="Back to Admin Panel")],
    ], resize_keyboard=True)

async def refer_admin(m):
    if m.from_user.id != ADMIN_ID: return
    state.pop(ADMIN_ID,None)
    return await ans(m, "Refer Management", reply_markup=refer_admin_kb())

async def refer_admin_action(m):
    if m.from_user.id != ADMIN_ID: return False
    t=clean_button_text(m.text)
    s=state.get(ADMIN_ID,{})
    if t == "Refer Management": return await refer_admin(m)
    if t == "Back to Admin Panel": state.pop(ADMIN_ID,None); return await ans(m,"Admin Panel",reply_markup=am())
    if t == "Refer Statistics":
        refs=c.execute("SELECT COUNT(*) n FROM referrals").fetchone()["n"]
        admins=c.execute("SELECT COUNT(DISTINCT admin_id) n FROM admin_commissions").fetchone()["n"]
        commissions=c.execute("SELECT COALESCE(SUM(commission),0) n FROM referral_commissions").fetchone()["n"]
        return await ans(m, f"Refer Statistics\n\nTotal referrals: {refs}\nAdmin referral users: {admins}\nTotal commission paid: {float(commissions or 0):.2f} BDT", reply_markup=refer_admin_kb())
    if t == "Enable/Disable User Commission":
        v=setting("ref_user_commission_enabled","1")
        save_setting("ref_user_commission_enabled", "0" if v=="1" else "1")
        return await ans(m, f"User commission is now {'ENABLED' if v!='1' else 'DISABLED'}.", reply_markup=refer_admin_kb())
    if t == "Enable/Disable Admin Commission":
        v=setting("ref_admin_commission_enabled","1")
        save_setting("ref_admin_commission_enabled", "0" if v=="1" else "1")
        return await ans(m, f"Admin commission is now {'ENABLED' if v!='1' else 'DISABLED'}.", reply_markup=refer_admin_kb())
    if t == "Set Default Commission":
        state[ADMIN_ID]={"action":"ref_default"}
        return await ans(m, "Format: fixed 0.05\nor\npercent 5\n\nDefault fixed amount is 0.05 BDT per Green ID.")
    if t == "Set Admin Green Commission":
        state[ADMIN_ID]={"action":"ref_admin_id"}
        return await ans(m, "Admin User ID দিন:")
    if t == "List Admin Commission":
        rows=c.execute("SELECT * FROM admin_commissions ORDER BY admin_id,category_id").fetchall()
        if not rows: return await ans(m,"কোনো Admin commission rule সেট করা হয়নি।",reply_markup=refer_admin_kb())
        lines=["Admin Commission List\n"]
        for r in rows:
            lines.append(f"Admin: {r['admin_id']} | Category: {r['category_name']} | {r['commission_type']} {float(r['value']):g} | {'ON' if r['enabled'] else 'OFF'}")
        return await ans(m,"\n".join(lines),reply_markup=refer_admin_kb())
    if s.get("action")=="ref_default":
        parts=m.text.strip().split()
        if len(parts)!=2 or parts[0].lower() not in {"fixed","percent"}:
            return await ans(m,"Format ভুল। Example: fixed 0.05 অথবা percent 5")
        try: val=float(parts[1])
        except: return await ans(m,"Commission value সঠিক সংখ্যা দিন।")
        if val<0 or (parts[0].lower()=="percent" and val>100): return await ans(m,"Value invalid.")
        save_setting("ref_default_commission_type",parts[0].lower()); save_setting("ref_default_commission_value",str(val)); state.pop(ADMIN_ID,None)
        return await ans(m,f"Default commission set: {parts[0].lower()} {val:g}",reply_markup=refer_admin_kb())
    if s.get("action")=="ref_admin_id":
        try: uid=int(m.text.strip())
        except: return await ans(m,"Valid numeric Admin User ID দিন।")
        state[ADMIN_ID]={"action":"ref_admin_category","admin_id":uid}
        rows=c.execute("SELECT * FROM categories ORDER BY id").fetchall()
        if not rows: state.pop(ADMIN_ID,None); return await ans(m,"কোনো category নেই।",reply_markup=refer_admin_kb())
        kb=ReplyKeyboardMarkup(keyboard=[[kbbtn(text=x['name'])] for x in rows]+[[kbbtn(text="Cancel")]],resize_keyboard=True)
        return await ans(m,"Category select করুন—এই category-এর Green ID-এর জন্য commission প্রযোজ্য হবে।",reply_markup=kb)
    if s.get("action")=="ref_admin_category":
        row=c.execute("SELECT * FROM categories WHERE name=?",(m.text.strip(),)).fetchone()
        if not row: return await ans(m,"Category পাওয়া যায়নি। আবার select করুন।")
        state[ADMIN_ID]={"action":"ref_admin_type","admin_id":s['admin_id'],'category_id':row['id'],'category_name':row['name']}
        return await ans(m,"Format: fixed 0.05\nor\npercent 5")
    if s.get("action")=="ref_admin_type":
        parts=m.text.strip().split()
        if len(parts)!=2 or parts[0].lower() not in {"fixed","percent"}:
            return await ans(m,"Format ভুল। Example: fixed 0.05 অথবা percent 5")
        try: val=float(parts[1])
        ...
        if val<0 or (parts[0].lower()=="percent" and val>100): return await ans(m,"Value invalid.")
        a=s['admin_id']; cat=s['category_id']; typ=parts[0].lower()
        c.execute("INSERT INTO admin_commissions(admin_id,category_id,category_name,commission_type,value,enabled,created_at,updated_at) VALUES(?,?,?,?,?,1,?,?) ON CONFLICT(admin_id,category_id) DO UPDATE SET category_name=excluded.category_name,commission_type=excluded.commission_type,value=excluded.value,enabled=1,updated_at=excluded.updated_at",(a,cat,s['category_name'],typ,val,now(),now()))
        db.commit(); state.pop(ADMIN_ID,None)
        return await ans(m,f"Admin commission saved.\nAdmin: {a}\nCategory: {s['category_name']}\nCommission: {typ} {val:g}",reply_markup=refer_admin_kb())
    return False
...
async def start(m):
    ...
    # Capture Telegram deep-link referral only once per referred user.
    existing = c.execute("SELECT 1 FROM referrals WHERE referred_id=?", (m.from_user.id,)).fetchone()
    payload = ""
    try:
        ...
        payload=""
    if not existing and payload.startswith("ref_"):
        try:
            ref_id=int(payload[4:])
            if ref_id != m.from_user.id and c.execute("SELECT 1 FROM users WHERE user_id=?", (ref_id,)).fetchone():
                c.execute("INSERT OR IGNORE INTO referrals(referred_id,referrer_id,created_at) VALUES(?,?,?)", (m.from_user.id,ref_id,now()))
                db.commit()
        except Exception:
            pass
    user(m.from_user)
...
CREATE TABLE IF NOT EXISTS referrals(referred_id INTEGER PRIMARY KEY,referrer_id INTEGER NOT NULL,created_at TEXT NOT NULL);
    CREATE TABLE IF NOT EXISTS referral_commissions(id INTEGER PRIMARY KEY AUTOINCREMENT,referrer_id INTEGER NOT NULL,referred_id INTEGER NOT NULL,category_id INTEGER DEFAULT 0,category_name TEXT DEFAULT '',uid TEXT NOT NULL,base_amount REAL DEFAULT 0,commission REAL DEFAULT 0,commission_type TEXT DEFAULT 'fixed',rate REAL DEFAULT 0,created_at TEXT NOT NULL,UNIQUE(referrer_id,referred_id,category_id,uid));
    CREATE TABLE IF NOT EXISTS admin_commissions(id INTEGER PRIMARY KEY AUTOINCREMENT,admin_id INTEGER NOT NULL,category_id INTEGER NOT NULL,category_name TEXT DEFAULT '',commission_type TEXT NOT NULL DEFAULT 'fixed',value REAL NOT NULL DEFAULT 0,enabled INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,UNIQUE(admin_id,category_id));
        """)
    migrations = [
...
dp.message.register(refer_section,F.text, lambda m: clean_button_text(m.text)=="Refer")
...
dp.message.register(refer_admin_action,F.from_user.id==ADMIN_ID, lambda m: clean_button_text(m.text) in {"Refer Management","Refer Statistics","Enable/Disable User Commission","Enable/Disable Admin Commission","Set Default Commission","Set Admin Green Commission","List Admin Commission","Back to Admin Panel"} or state.get(ADMIN_ID,{}).get("action","").startswith("ref_"))
