...
async def admin_summary(m):
    """Show a read-only operational overview of the bot from its database."""
    if m.from_user.id != ADMIN_ID:
        return
    ...
    try:
        ...
        active_users = c.execute("SELECT COUNT(*) AS n FROM users WHERE COALESCE(blocked,0)=0").fetchone()["n"]
        blocked_users = c.execute("SELECT COUNT(*) AS n FROM users WHERE COALESCE(blocked,0)=1").fetchone()["n"]
        total_balance = c.execute("SELECT COALESCE(SUM(balance),0) AS n FROM users").fetchone()["n"]
        total_income = c.execute("SELECT COALESCE(SUM(total_income),0) AS n FROM users").fetchone()["n"]

        # Work / submission statistics
        work_total = c.execute("SELECT COUNT(*) AS n FROM categories").fetchone()["n"]
        work_active = c.execute("SELECT COUNT(*) AS n FROM categories WHERE active=1").fetchone()["n"]
        work_disabled = work_total - work_active
        sub_total = c.execute("SELECT COUNT(*) AS n FROM submissions").fetchone()["n"]
        sub_pending = c.execute("SELECT COUNT(*) AS n FROM submissions WHERE status='pending'").fetchone()["n"]
        sub_approved = c.execute("SELECT COUNT(*) AS n FROM submissions WHERE status='approved'").fetchone()["n"]
        sub_rejected = c.execute("SELECT COUNT(*) AS n FROM submissions WHERE status='rejected'").fetchone()["n"]
        submitted_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) AS n FROM submissions").fetchone()["n"]
        approved_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) AS n FROM submissions WHERE status='approved'").fetchone()["n"]
        estimated_profit = c.execute("SELECT COALESCE(SUM(estimated_profit),0) AS n FROM submissions WHERE status='approved'").fetchone()["n"]

        # Withdrawal statistics
        wd_total = c.execute("SELECT COUNT(*) AS n FROM withdrawals").fetchone()["n"]
        wd_pending = c.execute("SELECT COUNT(*) AS n FROM withdrawals WHERE status='pending'").fetchone()["n"]
        wd_approved = c.execute("SELECT COUNT(*) AS n FROM withdrawals WHERE status='approved'").fetchone()["n"]
        wd_rejected = c.execute("SELECT COUNT(*) AS n FROM withdrawals WHERE status='rejected'").fetchone()["n"]
        wd_requested = c.execute("SELECT COALESCE(SUM(amount),0) AS n FROM withdrawals").fetchone()["n"]
        wd_paid = c.execute("SELECT COALESCE(SUM(amount),0) AS n FROM withdrawals WHERE status='approved'").fetchone()["n"]
        wd_pending_amount = c.execute("SELECT COALESCE(SUM(amount),0) AS n FROM withdrawals WHERE status='pending'").fetchone()["n"]
        wd_fees = c.execute("SELECT COALESCE(SUM(fee),0) AS n FROM withdrawals").fetchone()["n"]
        wd_methods = c.execute("SELECT COUNT(*) AS n FROM withdrawal_categories WHERE active=1").fetchone()["n"]

        # Report / payment-processing statistics
        report_count = c.execute("SELECT COUNT(*) AS n FROM reports").fetchone()["n"]
        paid_uid_count = c.execute("SELECT COUNT(*) AS n FROM paid_uids").fetchone()["n"]
        paid_uid_amount = c.execute("SELECT COALESCE(SUM(amount),0) AS n FROM paid_uids").fetchone()["n"]
        latest_report = c.execute("SELECT * FROM reports ORDER BY id DESC LIMIT 1").fetchone()

        # Referral statistics
        ref_users = c.execute("SELECT COUNT(*) AS n FROM users WHERE referrer_id IS NOT NULL").fetchone()["n"]
        ref_events = c.execute("SELECT COUNT(*) AS n FROM referral_commissions").fetchone()["n"]
        ref_paid = c.execute("SELECT COALESCE(SUM(amount),0) AS n FROM referral_commissions").fetchone()["n"]
        ref_rules = c.execute("SELECT COUNT(*) AS n FROM admin_referral_commissions WHERE active=1").fetchone()["n"]

        # Other bot configuration
        force_join_total = c.execute("SELECT COUNT(*) AS n FROM force_join_channels WHERE active=1").fetchone()["n"]
        custom_buttons = c.execute("SELECT COUNT(*) AS n FROM custom_buttons WHERE active=1").fetchone()["n"]
        support_buttons = c.execute("SELECT COUNT(*) AS n FROM support_buttons WHERE active=1").fetchone()["n"]
        tutorials = c.execute("SELECT COUNT(*) AS n FROM tutorials WHERE active=1").fetchone()["n"]

        report_line = "No report uploaded."
        if latest_report:
            report_line = (
                f"Latest Report: #{latest_report['id']} | {latest_report['file_name']}\n"
                f"Green IDs: {int(latest_report['green_count'] or 0):,} | "
                f"Processed Users: {int(latest_report['processed_users'] or 0):,} | "
                f"Reward: {float(latest_report['total_reward'] or 0):,.2f}"
            )
        ...
        text = (
            ...
            f"• Current User Balance: {float(total_balance or 0):,.2f}\n"
            f"• Total User Income: {float(total_income or 0):,.2f}\n\n"
            "📁 WORK / SUBMISSIONS\n"
            f"• Works: {work_total:,} | Active: {work_active:,} | Disabled: {work_disabled:,}\n"
            f"• Total Submissions: {sub_total:,}\n"
            f"• Pending: {sub_pending:,}\n"
            f"• Approved: {sub_approved:,}\n"
            f"• Rejected: {sub_rejected:,}\n"
            f"• Submitted Accounts: {int(submitted_accounts or 0):,}\n"
            f"• Approved Accounts: {int(approved_accounts or 0):,}\n"
            f"• Approved Profit: {float(estimated_profit or 0):,.2f}\n\n"
            "💳 WITHDRAWALS\n"
            f"• Total Requests: {wd_total:,}\n"
            f"• Pending: {wd_pending:,} ({float(wd_pending_amount or 0):,.2f})\n"
            f"• Approved/Paid: {wd_approved:,} ({float(wd_paid or 0):,.2f})\n"
            f"• Rejected: {wd_rejected:,}\n"
            f"• Total Requested: {float(wd_requested or 0):,.2f}\n"
            f"• Total Fees: {float(wd_fees or 0):,.2f}\n"
            f"• Active Withdrawal Methods: {wd_methods:,}\n\n"
            "📈 REPORT / PAYMENT PROCESSING\n"
            f"• Uploaded Reports: {report_count:,}\n"
            f"• Paid UIDs: {paid_uid_count:,}\n"
            f"• Paid UID Amount: {float(paid_uid_amount or 0):,.2f}\n"
            f"• {report_line}\n\n"
            "🎁 REFERRAL\n"
            f"• Referred Users: {ref_users:,}\n"
            f"• Commission Events: {ref_events:,}\n"
            f"• Total Commission: {float(ref_paid or 0):,.2f}\n"
            f"• Active Admin Rules: {ref_rules:,}\n\n"
            "⚙️ BOT CONFIGURATION\n"
            f"• Force Join Channels: {force_join_total:,}\n"
            f"• Custom Buttons: {custom_buttons:,}\n"
            f"• Support Buttons: {support_buttons:,}\n"
            f"• Tutorials: {tutorials:,}"
        )
        await ans(m, text, reply_markup=am())
    except Exception as e:
        print("[summary] error:", e)
        await ans(m, "❌ Summary তৈরি করতে সমস্যা হয়েছে।", reply_markup=am())
