...
def ibtn(text, **kwargs):
    ...
    try:
        ...
        kwargs.pop("style", None)
        return _InlineKeyboardButton(text=text, **kwargs)
TOKEN = "8856510779:AAF57bsElHzVrQFCdlNEPTcr1PEts42UJuA"
ADMIN_ID = 6995426618
...
async def referral_message(m):
    ...
    # Telegram's CopyTextButton copies the referral URL directly to the
    # user's clipboard. It must NOT be a URL button because that opens the
    # referral link instead of copying it.
    if CopyTextButton is None:
        return await ans(m, "❌ Copy Referral Button চালু করতে aiogram-এর নতুন version প্রয়োজন।", reply_markup=um(m.from_user.id))
    copy_btn = _InlineKeyboardButton(
        text="কপি রেফার লিংক",
    ...
    return await ans(
        m,
        text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[[copy_btn]])
    )
