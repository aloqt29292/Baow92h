...
def _admin_kbbtn(text):
    """Build an Admin Panel reply button without losing its color.

    Some aiogram/Bot API combinations reject the custom-emoji + style
    combination together.  The old kbbtn() fallback removed BOTH fields,
    which made the Admin Panel keyboard revert to plain white buttons after
    returning from Pending Withdrawals.  Here we keep style first, then add
    the premium icon only when the model accepts both fields.
    """
    clean = _remove_normal_emojis(text)
    icon, style = _button_meta(clean)
    try:
        return _KeyboardButton(text=clean, style=style, icon_custom_emoji_id=icon)
    except Exception:
        try:
            return _KeyboardButton(text=clean, style=style)
        except Exception:
            return _KeyboardButton(text=clean)

def am():
    return ReplyKeyboardMarkup(
        keyboard=[
            [_admin_kbbtn("User Management"),_admin_kbbtn("Work Management")],
            [_admin_kbbtn("Withdrawal Management")],
            [_admin_kbbtn("Force Join"),_admin_kbbtn("Pending Files")],
            [_admin_kbbtn("Upload Report File"),_admin_kbbtn("Generate Main Excel")],
            [_admin_kbbtn("Set Payment Channel")],
            [_admin_kbbtn("Set Report Channel")],
            [_admin_kbbtn("Set Receiver Channel")],
            [_admin_kbbtn("Broadcast"),_admin_kbbtn("New Button")],
            [_admin_kbbtn("Database"), _admin_kbbtn("Maintenance")],
            [_admin_kbbtn("Support Management")],
            [_admin_kbbtn("Refer Management")],
            [_admin_kbbtn("Tutorial Management")],
            [_admin_kbbtn("User Menu")]
        ],
        resize_keyboard=True,
    )
def cats():
...
async def admin(m):
    ...
    if t=="Generate Main Excel":
        return await main_excel_category_selector(m)
    if t=="New Button":
        state.pop(ADMIN_ID,None)
        return await ans(m, "🆕 New Button",reply_markup=new_button_admin_kb())
...
async def main_excel_category_selector(m):
    if m.from_user.id != ADMIN_ID:
        return
    rows = c.execute(
        "SELECT category_id, category_name, COUNT(*) AS total FROM submissions "
    ...
    buttons=[]
    for x in rows:
        cid = int(x["category_id"] or 0)
        name = x["category_name"] or f"Category {cid}"
        buttons.append([ibtn(text=f"{name} ({int(x['total'] or 0)} files)", callback_data=f"mainexcelcat:{cid}")])
    buttons.append([ibtn(text="Back to Admin Panel", callback_data="mainexcelcat:back", style="danger", icon_custom_emoji_id=PE.get("back"))])
    return await ans(
        m,
...
async def merge(m, category_id=None, admin_id=None):
    uid = admin_id if admin_id is not None else getattr(getattr(m, "from_user", None), "id", None)
    if uid != ADMIN_ID:
        return
    if category_id is None:
        rows=c.execute("SELECT * FROM submissions WHERE status!='rejected' ORDER BY id").fetchall()
    else:
        rows=c.execute("SELECT * FROM submissions WHERE status!='rejected' AND category_id=? ORDER BY id", (int(category_id),)).fetchall()
...
async def main_excel_category_callback(q):
    if q.from_user.id != ADMIN_ID:
        return await q.answer("Not allowed.", show_alert=True)
    data = q.data or ""
    if data == "mainexcelcat:back":
        await q.answer()
        try:
            return await q.message.edit_text("Admin Panel", reply_markup=None)
        finally:
            await ans(q.message, "Admin Panel", reply_markup=am())
    try:
        cid = int(data.split(":", 1)[1])
    except Exception:
        return await q.answer("Invalid category.", show_alert=True)
    row = c.execute("SELECT category_name FROM submissions WHERE category_id=? AND status!='rejected' ORDER BY id DESC LIMIT 1", (cid,)).fetchone()
    if not row:
        return await q.answer("এই category-তে কোনো submitted file নেই।", show_alert=True)
    await q.answer("Main Excel তৈরি হচ্ছে…")
    try:
        await q.message.edit_reply_markup(reply_markup=None)
    except Exception:
        pass
    return await merge(q.message, category_id=cid, admin_id=ADMIN_ID)
...
# Critical Admin Panel navigation handler.
async def pending_withdrawals_direct(m):
    """Handle both pending-withdrawal button labels from the Admin Panel."""
    if m.from_user.id != ADMIN_ID:
        return
    ...
    for x in rows:
        ...
        markup = InlineKeyboardMarkup(inline_keyboard=[[
            ibtn(text="Approve", callback_data=f"wapprove:{wid}", style="success", icon_custom_emoji_id=PE.get("done")),
            ibtn(text="Reject + Refund", callback_data=f"wreject:{wid}", style="danger", icon_custom_emoji_id=PE.get("cross"))
        ]])
