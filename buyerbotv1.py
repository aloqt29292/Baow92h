...
async def admin(m):
    ...
    if s.get("action")=="generate_main_excel_category":
        if t=="Back to Admin Panel":
            state.pop(ADMIN_ID,None)
            return await ans(m, "Admin Panel", reply_markup=am())
        match=c.execute(
            "SELECT category_name, COUNT(*) AS n FROM submissions "
            "WHERE status!='rejected' AND LOWER(TRIM(category_name))=LOWER(TRIM(?)) "
            "GROUP BY category_name LIMIT 1",
            (t,)
        ).fetchone()
        if not match:
            return await ans(m, "এই Category-এর কোনো submitted file পাওয়া যায়নি।", reply_markup=main_excel_categories_kb()[0])
        state.pop(ADMIN_ID,None)
        return await merge(m, match["category_name"])
    if t=="New Button":
        state.pop(ADMIN_ID,None)
        return await ans(m, "🆕 New Button",reply_markup=new_button_admin_kb())
...
def main_excel_categories_kb():
    # Only show categories that actually have submitted files available for
    # Main Excel generation. Keep disabled Works visible if they have data.
    rows=c.execute(
        "SELECT category_name, MIN(category_id) AS category_id, COUNT(*) AS n "
    ...
    for x in rows:
        ...
        if not name:
            continue
        keyboard.append([kbbtn(text=name, icon_custom_emoji_id=category_custom_emoji_id(name))])
    keyboard.append([kbbtn(text="Back to Admin Panel")])
    return ReplyKeyboardMarkup(keyboard=keyboard, resize_keyboard=True), rows
...
async def merge(m, category_name=None):
    ...
    if not category_name:
        ...
        return await ans(m, "Generate Main Excel\n\nযে Work/Category-এর Main Excel প্রয়োজন সেটি নির্বাচন করুন।", reply_markup=kb)
    rows=c.execute(
