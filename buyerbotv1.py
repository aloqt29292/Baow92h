...
def am():
    # Admin main panel uses inline buttons so Telegram can apply real button
    # styles (including the requested BLUE Work Management button).
    rows = [
        [ibtn(text="User Management", callback_data="adminpanel:User Management", style="primary"),
         ibtn(text="Work Management", callback_data="adminpanel:Work Management", style="primary")],
        [ibtn(text="Withdrawal Management", callback_data="adminpanel:Withdrawal Management", style="success")],
        [ibtn(text="Force Join", callback_data="adminpanel:Force Join", style="success"),
         ibtn(text="Pending Files", callback_data="adminpanel:Pending Files", style="success")],
        [ibtn(text="Upload Report File", callback_data="adminpanel:Upload Report File", style="danger"),
         ibtn(text="Generate Main Excel", callback_data="adminpanel:Generate Main Excel", style="danger")],
        [ibtn(text="Set Payment Channel", callback_data="adminpanel:Set Payment Channel", style="primary")],
        [ibtn(text="Set Report Channel", callback_data="adminpanel:Set Report Channel", style="primary")],
        [ibtn(text="Set Receiver Channel", callback_data="adminpanel:Set Receiver Channel", style="primary")],
        [ibtn(text="Broadcast", callback_data="adminpanel:Broadcast", style="success"),
         ibtn(text="New Button", callback_data="adminpanel:New Button", style="success")],
        [ibtn(text="Database", callback_data="adminpanel:Database", style="danger"),
         ibtn(text="Maintenance", callback_data="adminpanel:Maintenance", style="danger")],
        [ibtn(text="Support Management", callback_data="adminpanel:Support Management", style="primary")],
        [ibtn(text="Refer Management", callback_data="adminpanel:Refer Management", style="primary")],
        [ibtn(text="Tutorial Management", callback_data="adminpanel:Tutorial Management", style="primary")],
        [ibtn(text="User Menu", callback_data="adminpanel:User Menu", style="primary")],
    ]
...
def main_excel_categories_kb():
    # Main Excel category selector: only categories that have submitted files.
    # Use callback buttons so selecting a category cannot be swallowed by the
    # generic admin text handler and always targets the exact category ID.
    rows=c.execute(
        "SELECT category_id, category_name, COUNT(*) AS n "
        "FROM submissions WHERE status!='rejected' AND TRIM(COALESCE(category_name,''))!='' "
        "GROUP BY category_id, category_name ORDER BY category_name COLLATE NOCASE"
    ).fetchall()
    keyboard=[]
    for x in rows:
        name=str(x["category_name"] or "").strip()
        if not name:
            continue
        cid=x["category_id"]
        keyboard.append([ibtn(text=name, callback_data=f"mainexcelcat:{cid}", icon_custom_emoji_id=category_custom_emoji_id(name), style="primary")])
    keyboard.append([ibtn(text="Back to Admin Panel", callback_data="mainexcelcat:back", style="danger")])
    return InlineKeyboardMarkup(inline_keyboard=keyboard), rows

async def merge(m, category_name=None):
    if m.from_user.id!=ADMIN_ID:return
    if not category_name:
        kb,rows=main_excel_categories_kb()
        if not rows:
            return await ans(m, "কোনো submitted file নেই।", reply_markup=am())
        state[ADMIN_ID]={"action":"generate_main_excel_category"}
        return await ans(m, "Generate Main Excel\n\nযে Work/Category-এর Main Excel প্রয়োজন সেটি নির্বাচন করুন।\n\nনিচ থেকে একটি Category select করুন:", reply_markup=kb)
    rows=c.execute(
        "SELECT * FROM submissions WHERE status!='rejected' AND category_name=? ORDER BY id",
        (category_name,)
    ).fetchall()
...
class _CallbackMessageProxy:
    """Small Message-compatible proxy for routing inline admin buttons into
    the existing message-based admin functions without duplicating workflows."""
    ...
    async def answer(self, *args, **kwargs):
        return await self._q.message.answer(*args, **kwargs)

async def admin_panel_callback(q):
    if q.from_user.id != ADMIN_ID:
        return await q.answer("Not allowed.", show_alert=True)
    data=q.data or ""
    if not data.startswith("adminpanel:"):
        return
    text=data.split(":",1)[1]
    await q.answer()
    m=_CallbackMessageProxy(q, text)
    # These buttons have dedicated handlers; route them directly.
    if text == "Force Join":
        return await force_join_management(m)
    if text == "Pending Files":
        return await admin(m)
    if text == "Upload Report File":
        return await report_admin_action(m)
    if text == "Database":
        return await database_admin_action(m)
    if text == "Maintenance":
        return await maintenance_admin_action(m)
    if text == "Refer Management":
        return await referral_admin_action(m)
    if text == "Tutorial Management":
        return await tutorial_admin_action(m)
    if text == "User Menu":
        return await admin(m)
    return await admin(m)

async def main_excel_category_callback(q):
    if q.from_user.id != ADMIN_ID:
        return await q.answer("Not allowed.", show_alert=True)
    data=q.data or ""
    if not data.startswith("mainexcelcat:"):
        return
    value=data.split(":",1)[1]
    if value == "back":
        state.pop(ADMIN_ID,None)
        await q.answer()
        return await q.message.answer("Admin Panel", reply_markup=am())
    try:
        category_id=int(value)
    except ValueError:
        return await q.answer("Invalid category.", show_alert=True)
    row=c.execute(
        "SELECT category_name FROM submissions WHERE id IN "
        "(SELECT MIN(id) FROM submissions WHERE category_id=? AND status!='rejected')",
        (category_id,)
    ).fetchone()
    if not row:
        # Legacy submissions may have category_id=0; fall back to an exact
        # category lookup by ID in categories when possible.
        row=c.execute("SELECT name FROM categories WHERE id=?",(category_id,)).fetchone()
    if not row:
        return await q.answer("এই Category-এর কোনো submitted file পাওয়া যায়নি।", show_alert=True)
    category_name=str(row["category_name"] if "category_name" in row.keys() else row["name"])
    await q.answer()
    state.pop(ADMIN_ID,None)
    return await merge(q.message, category_name)
...
dp.callback_query.register(admin_panel_callback,F.data.startswith("adminpanel:"))
dp.callback_query.register(main_excel_category_callback,F.data.startswith("mainexcelcat:"))
