...
def _button_meta(text):
    ...
    rules = [
        ...
        (("samari", "summary"), "admin_generate_excel", "primary"),
...
def _day_utc_bounds(day):
    """Return UTC timestamp strings for a Bangladesh calendar day."""
    start_local = datetime(day.year, day.month, day.day, 0, 0, 0, tzinfo=BD_TZ)
    end_local = start_local + timedelta(days=1)
    return start_local.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M:%S"), end_local.astimezone(timezone.utc).strftime("%Y-%m-%d %H:%M:%S")
...
def build_summary(report_day=None):
    """Build one large admin summary. If report_day is given, its Bangladesh-day activity is shown."""
    if report_day is None:
        report_day = datetime.now(BD_TZ).date()
    start_utc, end_utc = _day_utc_bounds(report_day)
    prev_day = report_day - timedelta(days=1)
    prev_start_utc, prev_end_utc = _day_utc_bounds(prev_day)

    total_users = c.execute("SELECT COUNT(*) n FROM users").fetchone()["n"]
    active_users = c.execute("SELECT COUNT(*) n FROM users WHERE COALESCE(blocked,0)=0").fetchone()["n"]
    blocked_users = c.execute("SELECT COUNT(*) n FROM users WHERE COALESCE(blocked,0)=1").fetchone()["n"]
    total_balance = c.execute("SELECT COALESCE(SUM(balance),0) n FROM users").fetchone()["n"]
    total_income = c.execute("SELECT COALESCE(SUM(total_income),0) n FROM users").fetchone()["n"]
    total_referrals = c.execute("SELECT COUNT(*) n FROM users WHERE referrer_id IS NOT NULL AND referrer_id != 0").fetchone()["n"]
    referral_bonus = c.execute("SELECT COALESCE(SUM(amount),0) n FROM referral_commissions").fetchone()["n"]

    new_users = c.execute("SELECT COUNT(*) n FROM users WHERE created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    new_users_prev = c.execute("SELECT COUNT(*) n FROM users WHERE created_at >= ? AND created_at < ?", (prev_start_utc,prev_end_utc)).fetchone()["n"]

    total_submissions = c.execute("SELECT COUNT(*) n FROM submissions").fetchone()["n"]
    pending_files = c.execute("SELECT COUNT(*) n FROM submissions WHERE status='pending'").fetchone()["n"]
    approved_files = c.execute("SELECT COUNT(*) n FROM submissions WHERE status='approved'").fetchone()["n"]
    rejected_files = c.execute("SELECT COUNT(*) n FROM submissions WHERE status='rejected'").fetchone()["n"]
    total_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) n FROM submissions").fetchone()["n"]
    approved_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) n FROM submissions WHERE status='approved'").fetchone()["n"]
    available_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) n FROM submissions WHERE status='pending'").fetchone()["n"]

    total_sales = c.execute("SELECT COALESCE(SUM(estimated_profit),0) n FROM submissions WHERE status='approved'").fetchone()["n"]
    total_withdrawn = c.execute("SELECT COALESCE(SUM(amount),0) n FROM withdrawals WHERE status='approved'").fetchone()["n"]
    pending_withdraw_count = c.execute("SELECT COUNT(*) n FROM withdrawals WHERE status='pending'").fetchone()["n"]
    pending_withdraw_amount = c.execute("SELECT COALESCE(SUM(amount),0) n FROM withdrawals WHERE status='pending'").fetchone()["n"]
    total_withdraw_requests = c.execute("SELECT COUNT(*) n FROM withdrawals").fetchone()["n"]
    approved_withdraw_requests = c.execute("SELECT COUNT(*) n FROM withdrawals WHERE status='approved'").fetchone()["n"]
    rejected_withdraw_requests = c.execute("SELECT COUNT(*) n FROM withdrawals WHERE status='rejected'").fetchone()["n"]

    day_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) n FROM submissions WHERE created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_submissions = c.execute("SELECT COUNT(*) n FROM submissions WHERE created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_approved_accounts = c.execute("SELECT COALESCE(SUM(rows_count),0) n FROM submissions WHERE status='approved' AND created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_approved_amount = c.execute("SELECT COALESCE(SUM(estimated_profit),0) n FROM submissions WHERE status='approved' AND created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_withdraw_requests = c.execute("SELECT COUNT(*) n FROM withdrawals WHERE created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_withdraw_amount = c.execute("SELECT COALESCE(SUM(amount),0) n FROM withdrawals WHERE status='approved' AND created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_referrals = c.execute("SELECT COUNT(*) n FROM users WHERE referrer_id IS NOT NULL AND referrer_id != 0 AND created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]
    day_referral_bonus = c.execute("SELECT COALESCE(SUM(amount),0) n FROM referral_commissions WHERE created_at >= ? AND created_at < ?", (start_utc,end_utc)).fetchone()["n"]

    active_works = c.execute("SELECT COUNT(*) n FROM categories WHERE active=1").fetchone()["n"]
    total_works = c.execute("SELECT COUNT(*) n FROM categories").fetchone()["n"]

    work_rows = c.execute("""
        SELECT category_name,
               COALESCE(SUM(rows_count),0) accounts,
               COALESCE(SUM(CASE WHEN status='pending' THEN rows_count ELSE 0 END),0) pending_accounts,
               COALESCE(SUM(CASE WHEN status='approved' THEN rows_count ELSE 0 END),0) approved_accounts,
               COALESCE(SUM(CASE WHEN status='rejected' THEN rows_count ELSE 0 END),0) rejected_accounts,
               COUNT(*) submissions,
               COALESCE(SUM(CASE WHEN status='approved' THEN estimated_profit ELSE 0 END),0) approved_value
        FROM submissions GROUP BY category_name ORDER BY category_name COLLATE NOCASE
    """).fetchall()

    top_ref = c.execute("""
        SELECT u.referrer_id, u.username, COUNT(*) cnt
        FROM users u WHERE u.referrer_id IS NOT NULL AND u.referrer_id != 0
        GROUP BY u.referrer_id ORDER BY cnt DESC LIMIT 1
    """).fetchone()
    top_ref_text = "N/A"
    if top_ref:
        top_ref_text = f"{('@'+top_ref['username']) if top_ref['username'] else top_ref['referrer_id']} ({top_ref['cnt']})"
    ...
    lines = [
        ...
        "╚══════════════════════════════╝",
        "",
        f"Report Date: {report_day.strftime('%d %b %Y')} (Bangladesh)",
        "",
        "[ USER SUMMARY ]",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        ...
        f"Previous Day New     : {new_users_prev:,}",
        f"Total User Balance   : {_money(total_balance)}",
        f"Total User Income    : {_money(total_income)}",
        "",
        "[ WORK SUMMARY ]",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"Total Works          : {total_works:,}",
        f"Active Works         : {active_works:,}",
        f"Total Submissions    : {total_submissions:,}",
        f"Pending Files        : {pending_files:,}",
        f"Approved Files       : {approved_files:,}",
        f"Rejected Files       : {rejected_files:,}",
        f"Submitted Accounts   : {total_accounts:,}",
        f"Approved Accounts    : {approved_accounts:,}",
        f"Pending Accounts     : {available_accounts:,}",
        "",
        "[ FINANCIAL SUMMARY ]",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"Total Approved Sales: {_money(total_sales)}",
        f"Total Withdrawn     : {_money(total_withdrawn)}",
        f"Pending Withdraw    : {_money(pending_withdraw_amount)}",
        f"Total Balance Held  : {_money(total_balance)}",
        "",
        "[ WITHDRAWAL SUMMARY ]",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"Total Requests      : {total_withdraw_requests:,}",
        f"Pending Requests    : {pending_withdraw_count:,}",
        f"Approved Requests   : {approved_withdraw_requests:,}",
        f"Rejected Requests   : {rejected_withdraw_requests:,}",
        "",
        "[ REFERRAL SUMMARY ]",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"Total Referrals     : {total_referrals:,}",
        f"Referral Bonus Paid : {_money(referral_bonus)}",
        f"Top Referrer        : {top_ref_text}",
        "",
        "[ 24 HOUR ACTIVITY ]",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"New Users           : {new_users:,}",
        f"Submitted Files     : {day_submissions:,}",
        f"Submitted Accounts  : {day_accounts:,}",
        f"Approved Accounts   : {day_approved_accounts:,}",
        f"Approved Earnings   : {_money(day_approved_amount)}",
        f"Withdrawal Requests : {day_withdraw_requests:,}",
        f"Withdrawn Amount    : {_money(day_withdraw_amount)}",
        f"New Referrals       : {day_referrals:,}",
        f"Referral Bonus      : {_money(day_referral_bonus)}",
    ]

    if work_rows:
        lines += ["", "[ WORK-WISE DETAILS ]", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"]
        for w in work_rows:
            name=(w['category_name'] or 'Unknown').strip()
            lines += [
                f"{name}",
                f"  Accounts: {int(w['accounts'] or 0):,} | Approved: {int(w['approved_accounts'] or 0):,}",
                f"  Pending: {int(w['pending_accounts'] or 0):,} | Rejected: {int(w['rejected_accounts'] or 0):,}",
                f"  Submissions: {int(w['submissions'] or 0):,} | Value: {_money(w['approved_value'])}",
            ]

    lines += [
        "",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"Generated: {datetime.now(BD_TZ).strftime('%d %b %Y, %I:%M:%S %p')}",
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
    ]
    return "\n".join(lines)


async def send_daily_summary_loop(bot):
    """Push the previous Bangladesh calendar day's summary at each local midnight."""
    while True:
        try:
            now_local = datetime.now(BD_TZ)
            tomorrow = now_local.date() + timedelta(days=1)
            next_midnight = datetime(tomorrow.year, tomorrow.month, tomorrow.day, 0, 0, 0, tzinfo=BD_TZ)
            wait_seconds = max(1, (next_midnight - now_local).total_seconds())
            await asyncio.sleep(wait_seconds)
            report_day = datetime.now(BD_TZ).date() - timedelta(days=1)
            text = build_summary(report_day)
            await sendmsg(bot, ADMIN_ID, text, reply_markup=am())
        except asyncio.CancelledError:
            raise
        except Exception as e:
            print(f"[samari] daily summary error: {e}")
...
async def admin(m):
    ...
    if t in {"Samari", "Summary"}:
        state.pop(ADMIN_ID, None)
        return await ans(m, build_summary(), reply_markup=am())
...
async def main():
    ...
    asyncio.create_task(send_daily_summary_loop(b))
