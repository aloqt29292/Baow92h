...
async def referral_section(m):
    # Always answer the Referral reply-button explicitly. This keeps the feature
    # working even when the user arrives here after another menu/state action.
    ...
    if not username:
        return await ans(m,"Bot username পাওয়া যায়নি। Admin থেকে bot-এর username সেট করুন।",reply_markup=um(m.from_user.id))
    link=referral_link(m.from_user.id,username)

    rows=[]
    # Modern Telegram clients support one-tap copy through copy_text.
    try:
        from aiogram.types import CopyTextButton
        rows.append([InlineKeyboardButton(text="🔗 Copy Referral Link", copy_text=CopyTextButton(text=link))])
    except Exception:
        pass
    # Keep an always-working fallback so users can still open and use the link.
    rows.append([InlineKeyboardButton(text="↗️ Open Referral Link", url=link)])
    rows.append([InlineKeyboardButton(text="📊 Referral Statistics", callback_data="ref_stats")])
    kb=InlineKeyboardMarkup(inline_keyboard=rows)
    await ans(m,"Referral Center\n\nনিচের বাটন থেকে Referral Link copy/open করুন অথবা Referral Statistics দেখুন।",reply_markup=kb)
...
dp.message.register(referral_section, lambda m: bool(m.text) and clean_button_text(m.text).strip()=="Referral")
