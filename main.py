...
async def referral_section(m):
    ...
    try:
        username=((await m.bot.get_me()).username or "").strip().lstrip("@")
    except Exception:
        return await ans(m,"Referral link তৈরি করা যায়নি। একটু পরে আবার চেষ্টা করুন।",reply_markup=um(m.from_user.id))
    if not username:
        return await ans(m,"Bot username সেট করা নেই, তাই Referral link তৈরি করা যাচ্ছে না।",reply_markup=um(m.from_user.id))
    link=referral_link(m.from_user.id,username)
    rows=[[InlineKeyboardButton(text="🔗 Open Referral Link", url=link)]]
    try:
    ...
    await ans(m,"Referral Center\n\nনিচের বাটন থেকে আপনার Referral Link copy/open করুন অথবা Referral Statistics দেখুন।",reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
...
dp.message.register(referral_section, F.text, lambda m: clean_button_text(m.text).strip().lower() in {"referral", "refer", "reffer"})
dp.message.register(referral_section, Command("referral"))
dp.message.register(referral_section, Command("refer"))
