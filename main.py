...
# ==============================
TOKEN = "8856510779:AAECLJ9wWLPVBf7bEKuyAOMjYO7n7tfXL38"
ADMIN_ID = 6995426618
# ==============================
...
CREATE TABLE IF NOT EXISTS referral_access(user_id INTEGER PRIMARY KEY, enabled INTEGER DEFAULT 0, commission_percent REAL DEFAULT 0, fixed_per_green REAL DEFAULT 0, created_at TEXT DEFAULT '');
CREATE TABLE IF NOT EXISTS referrals(id INTEGER PRIMARY KEY AUTOINCREMENT, referrer_id INTEGER NOT NULL, referred_id INTEGER UNIQUE NOT NULL, created_at TEXT DEFAULT '');
CREATE TABLE IF NOT EXISTS referral_commissions(id INTEGER PRIMARY KEY AUTOINCREMENT, referrer_id INTEGER NOT NULL, referred_id INTEGER NOT NULL, source_amount REAL DEFAULT 0, commission_amount REAL DEFAULT 0, source_type TEXT DEFAULT '', created_at TEXT DEFAULT '');
CREATE TABLE IF NOT EXISTS referral_category_commissions(referrer_id INTEGER NOT NULL, category_name TEXT NOT NULL, fixed_amount REAL DEFAULT 0, PRIMARY KEY(referrer_id, category_name));
"""); db.commit()
...
try:
    ...
    pass
try:
    c.execute("ALTER TABLE referral_access ADD COLUMN fixed_per_green REAL DEFAULT 0")
    db.commit()
except sqlite3.OperationalError:
    pass
...
async def process_report(bot, report_id, report_raw, category_name=""):
    ...
    for user_id, submissions in user_files.items():
        ...
        if ok_count:
            ...
            total_reward += reward
            # Referral owner receives fixed commission for every successful Green UID.
            add_referral_commission(user_id, reward, source_type="green_report", units=ok_count, category_name=category_name)
...
def um(uid):
    ...
    ref=c.execute("SELECT enabled FROM referral_access WHERE user_id=?",(uid,)).fetchone()
    if ref and int(ref["enabled"] or 0)==1:
        k.append([kbbtn(text="Referral")])
    if uid==ADMIN_ID:
...
def am():
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [kbbtn(text="Referral Management")],
...
async def start(m):
    ...
    user(m.from_user)
    # Referral binding is only accepted from users whose referral access is enabled.
    parts=(m.text or "").split(maxsplit=1)
    if len(parts)>1 and parts[1].startswith("ref_"):
        try:
            referrer_id=int(parts[1][4:])
            if referrer_id!=m.from_user.id:
                access=c.execute("SELECT enabled FROM referral_access WHERE user_id=?",(referrer_id,)).fetchone()
                if access and int(access["enabled"] or 0)==1:
                    c.execute("INSERT OR IGNORE INTO referrals(referrer_id,referred_id,created_at) VALUES(?,?,?)",(referrer_id,m.from_user.id,now())); db.commit()
...
def add_referral_commission(referred_id, amount, source_type="income", units=1, category_name=""):
    rel=c.execute("SELECT referrer_id FROM referrals WHERE referred_id=?",(referred_id,)).fetchone()
    if not rel: return 0.0
    referrer_id=rel["referrer_id"]
    access=c.execute("SELECT enabled,commission_percent,fixed_per_green FROM referral_access WHERE user_id=?",(referrer_id,)).fetchone()
    if not access or int(access["enabled"] or 0)!=1: return 0.0
    # Category-specific fixed amount takes priority for Green reports.
    fixed=0.0
    if category_name:
        row=c.execute("SELECT fixed_amount FROM referral_category_commissions WHERE referrer_id=? AND category_name=?",(referrer_id,category_name)).fetchone()
        if row: fixed=max(0.0,float(row["fixed_amount"] or 0))
        else: return 0.0  # This category is not eligible for this referrer.
    else:
        fixed=max(0.0,float(access["fixed_per_green"] or 0))
    pct=max(0.0,float(access["commission_percent"] or 0))
    commission=float(amount)*pct/100.0 + fixed*max(0,int(units or 0))
    if commission<=0: return 0.0
    c.execute("UPDATE users SET balance=balance+?,total_income=total_income+?,today_income=today_income+? WHERE user_id=?",(commission,commission,commission,referrer_id))
    c.execute("INSERT INTO referral_commissions(referrer_id,referred_id,source_amount,commission_amount,source_type,created_at) VALUES(?,?,?,?,?,?)",(referrer_id,referred_id,float(amount),commission,source_type,now()))
    db.commit(); return commission

async def referral_stats_text(uid):
    access=c.execute("SELECT * FROM referral_access WHERE user_id=?",(uid,)).fetchone()
    total=c.execute("SELECT COUNT(*) n FROM referrals WHERE referrer_id=?",(uid,)).fetchone()["n"]
    active=c.execute("SELECT COUNT(DISTINCT referred_id) n FROM referral_commissions WHERE referrer_id=?",(uid,)).fetchone()["n"]
    earned=c.execute("SELECT COALESCE(SUM(commission_amount),0) x FROM referral_commissions WHERE referrer_id=?",(uid,)).fetchone()["x"]
    cats=c.execute("SELECT category_name,fixed_amount FROM referral_category_commissions WHERE referrer_id=? ORDER BY category_name",(uid,)).fetchall()
    cat_text="\n".join(f"• {x['category_name']}: ৳{float(x['fixed_amount']):.2f} per Green" for x in cats) or "No category commission set"
    return (f"Referral Statistics\n\n👥 Total Referred Users: {total}\n🟢 Active/Successful Referrals: {active}\n💰 Total Commission Earned: ৳{float(earned or 0):.2f}\n📊 Percentage Commission: {float(access['commission_percent'] or 0):.2f}%\n\n📂 Eligible Category Commission:\n{cat_text}")

async def referral_section(m):
    """Open Referral Center reliably from the Reply Keyboard button."""
    try:
        user(m.from_user)
        access=c.execute("SELECT enabled FROM referral_access WHERE user_id=?",(m.from_user.id,)).fetchone()
        if not access or int(access["enabled"] or 0) != 1:
            return await ans(m, "Referral access আপনার জন্য enabled করা হয়নি।", reply_markup=um(m.from_user.id))

        try:
            bot_me = await m.bot.get_me()
            username = (bot_me.username or "").strip()
        except Exception as e:
            print(f"[referral] get_me error: {e}")
            username = ""

        if not username:
            return await ans(m, "Referral Link তৈরি করা যাচ্ছে না। Bot-এর Telegram username সেট করা নেই।", reply_markup=um(m.from_user.id))

        link = referral_link(m.from_user.id, username)
        rows = [
            [InlineKeyboardButton(text="🔗 Open Referral Link", url=link)],
            [InlineKeyboardButton(text="📊 Referral Statistics", callback_data="ref_stats")]
        ]
        # CopyTextButton is optional and may not exist on older aiogram/Telegram APIs.
        try:
            from aiogram.types import CopyTextButton
            rows.insert(0, [InlineKeyboardButton(text="📋 Copy Referral Link", copy_text=CopyTextButton(text=link))])
        except Exception as e:
            print(f"[referral] copy button unavailable: {e}")

        kb = InlineKeyboardMarkup(inline_keyboard=rows)
        return await ans(m, "🎁 Referral Center\n\nনিচের বাটন থেকে আপনার Referral Link ব্যবহার করুন।", reply_markup=kb)
    except Exception as e:
        print(f"[referral] handler error: {e}")
        return await ans(m, "Referral Center খুলতে সমস্যা হয়েছে। আবার চেষ্টা করুন।", reply_markup=um(m.from_user.id))

async def referral_stats_callback(q):
    if q.from_user.id is None: return
    access=c.execute("SELECT enabled FROM referral_access WHERE user_id=?",(q.from_user.id,)).fetchone()
    if not access or int(access["enabled"] or 0)!=1:
        return await q.answer("Referral access নেই।",show_alert=True)
    await q.answer()
    await q.message.answer(await referral_stats_text(q.from_user.id))

def referral_admin_kb():
    return ReplyKeyboardMarkup(keyboard=[[kbbtn(text="Enable Referral User"),kbbtn(text="Disable Referral User")],[kbbtn(text="Set Referral Commission"),kbbtn(text="Set Category Commission")],[kbbtn(text="Referral User List")],[kbbtn(text="Back to Admin Panel")]],resize_keyboard=True)

async def referral_admin_action(m):
    if m.from_user.id!=ADMIN_ID:return
    t=clean_button_text(m.text); st=state.get(ADMIN_ID,{})
    if t=="Referral Management":
        state.pop(ADMIN_ID,None); return await ans(m,"Referral Management",reply_markup=referral_admin_kb())
    if t=="Back to Admin Panel":
        state.pop(ADMIN_ID,None); return await ans(m,"Admin Panel",reply_markup=am())
    if t=="Enable Referral User": state[ADMIN_ID]={"action":"ref_enable_uid"}; return await ans(m,"Target User ID পাঠান।")
    if t=="Disable Referral User": state[ADMIN_ID]={"action":"ref_disable_uid"}; return await ans(m,"Target User ID পাঠান।")
    if t=="Set Referral Commission": state[ADMIN_ID]={"action":"ref_comm_uid"}; return await ans(m,"Target User ID পাঠান।")
    if t=="Set Category Commission": state[ADMIN_ID]={"action":"ref_cat_uid"}; return await ans(m,"Target User ID পাঠান।")
    if t=="Referral User List":
        rows=c.execute("SELECT r.user_id,u.username FROM referral_access r LEFT JOIN users u ON u.user_id=r.user_id WHERE r.enabled=1 ORDER BY r.user_id").fetchall()
        text="Referral Users\n\n"+("\n".join(f"ID: {x['user_id']} | @{x['username'] or 'N/A'}" for x in rows) if rows else "কোনো enabled referral user নেই।")
        return await ans(m,text,reply_markup=referral_admin_kb())
    a=st.get("action")
    try:
        if a=="ref_enable_uid":
            uid=int(t); exists=c.execute("SELECT 1 FROM users WHERE user_id=?",(uid,)).fetchone()
            if not exists: return await ans(m,"এই User ID এখনো bot ব্যবহার করেনি।",reply_markup=referral_admin_kb())
            c.execute("INSERT INTO referral_access(user_id,enabled,commission_percent,fixed_per_green,created_at) VALUES(?,1,0,0,?) ON CONFLICT(user_id) DO UPDATE SET enabled=1",(uid,now())); db.commit(); state.pop(ADMIN_ID,None)
            return await ans(m,f"Referral enabled for {uid}.",reply_markup=referral_admin_kb())
        if a=="ref_disable_uid":
            uid=int(t); c.execute("UPDATE referral_access SET enabled=0 WHERE user_id=?",(uid,)); db.commit(); state.pop(ADMIN_ID,None); return await ans(m,f"Referral disabled for {uid}.",reply_markup=referral_admin_kb())
        if a=="ref_comm_uid": state[ADMIN_ID]={"action":"ref_comm_percent","uid":int(t)}; return await ans(m,"Commission percentage পাঠান। উদাহরণ: 10")
        if a=="ref_comm_percent":
            pct=float(t); uid=st["uid"]
            if pct<0 or pct>100: raise ValueError()
            c.execute("INSERT INTO referral_access(user_id,enabled,commission_percent,created_at) VALUES(?,1,?,?) ON CONFLICT(user_id) DO UPDATE SET commission_percent=excluded.commission_percent",(uid,pct,now())); db.commit(); state.pop(ADMIN_ID,None)
            return await ans(m,f"User {uid} commission fixed: {pct:.2f}%",reply_markup=referral_admin_kb())
        if a=="ref_cat_uid": state[ADMIN_ID]={"action":"ref_cat_name","uid":int(t)}; return await ans(m,"যে Work/Category-এর Green ID-তে commission দিতে চান তার exact নাম পাঠান।")
        if a=="ref_cat_name":
            cat=t.strip(); exists=c.execute("SELECT 1 FROM categories WHERE name=?",(cat,)).fetchone()
            if not exists: return await ans(m,"এই নামে কোনো category নেই। exact category name পাঠান।")
            state[ADMIN_ID]={"action":"ref_cat_amount","uid":st["uid"],"category":cat}; return await ans(m,f"{cat} category-এর প্রতি Green ID-তে কত টাকা commission? উদাহরণ: 1")
        if a=="ref_cat_amount":
            amount=float(t); uid=st["uid"]; cat=st["category"]
            if amount<0: raise ValueError()
            c.execute("INSERT INTO referral_category_commissions(referrer_id,category_name,fixed_amount) VALUES(?,?,?) ON CONFLICT(referrer_id,category_name) DO UPDATE SET fixed_amount=excluded.fixed_amount",(uid,cat,amount)); db.commit(); state.pop(ADMIN_ID,None)
            return await ans(m,f"User {uid}: {cat} → ৳{amount:.2f} per Green ID",reply_markup=referral_admin_kb())
    except ValueError:
        return await ans(m,"সঠিক User ID অথবা valid amount দিন।",reply_markup=referral_admin_kb())
...
dp.message.register(referral_admin_action,F.from_user.id == ADMIN_ID, lambda m: clean_button_text(m.text) in {"Referral Management","Enable Referral User","Disable Referral User","Set Referral Commission","Set Category Commission","Referral User List","Back to Admin Panel"} or state.get(ADMIN_ID,{}).get("action","").startswith("ref_"))
dp.callback_query.register(referral_stats_callback, F.data=="ref_stats")
dp.message.register(referral_section, F.text, lambda m: clean_button_text(m.text or "").strip().casefold()=="referral")
