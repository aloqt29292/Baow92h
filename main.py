...
# ==================== CONSOLE MODULE (isolated storage) ====================
# Console uses the SAME bot token/admin identity as the main bot, but NEVER
# opens, reads, writes, migrates, or shares the main bot database.
DEFAULT_DATA = {
    "request_id": 14,
    "templates": [],
}
...
def console_load_data():
    if not CONSOLE_DATA_FILE.exists():
        console_save_data(DEFAULT_DATA.copy())
...
def console_save_data(d):
    CONSOLE_DATA_FILE.write_text(json.dumps(d, ensure_ascii=False, indent=2), encoding="utf-8")

def is_admin(uid):
    return uid == ADMIN_ID

def normalize_template(t, index=0):
    if isinstance(t, str):
        return {
...
def main_menu():
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton("Template Manager")],
            [KeyboardButton("Request ID"), KeyboardButton("Status")],
            [KeyboardButton("Back")],
        ],
...
def template_menu():
    ...
    for i, t in enumerate(d["templates"]):
        ...
        rows.append([KeyboardButton(f"Template {i+1} [{state}]")])
    rows.append([KeyboardButton("Add Template")])
    if d["templates"]:
        rows.append([KeyboardButton("Delete Template")])
...
def settings_menu():
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton("Set Channel"), KeyboardButton("Set Time")],
            [KeyboardButton("Set 10-Min Limit"), KeyboardButton("Set Amount Rules")],
            [KeyboardButton("Set Breakage %"), KeyboardButton("Set Complete %")],
            [KeyboardButton("ON/OFF")],
            [KeyboardButton("View Template")],
            [KeyboardButton("Back")],
        ],
        resize_keyboard=True,
        is_persistent=True,
...
def console_status_text():
    d = console_ensure_templates(console_load_data())
    lines = ["Console Message Bot Status", ""]
    lines.append(f"Templates: {len(d['templates'])}")
    lines.append(f"Next Request ID: #{int(d['request_id']) + 1}")
    if not d["templates"]:
...
def console_generate_template(text, template=None):
    values = {}
    if template:
    ...
    result = re.sub(r"\{submitted:(\d+)-(\d+)\}", sub, text, flags=re.I)
    ...
    def weighted_amount(m):
        ...
        # Choose one configured amount range by its probability, then pick
        # the actual amount from that range. This makes the percentages
        # control the real generated amount.
        selected = random.choices(valid_rules, weights=[r[2] for r in valid_rules], k=1)[0]
        rlo, rhi, _ = selected

        # Keep the placeholder's own range as a safety boundary.
        final_lo = max(lo, rlo)
        final_hi = min(hi, rhi)
        ...
        if bp + cp > 0:
            values["workflow_type"] = random.choices(
                ("breakage", "complete"), weights=(bp, cp), k=1
            )[0]
        else:
    ...
    result = re.sub(r"\{amount\}", lambda _: str(rr(10, 9999)), result, flags=re.I)
...
def console_kb(rows):
    return ReplyKeyboardMarkup(keyboard=[[kbbtn(text=x) for x in row] for row in rows], resize_keyboard=True)

def console_main_kb():
    return console_kb([["Template Manager"],["Request ID","Status"],["Back to Admin Panel"]])

def console_template_kb():
    d=console_ensure_templates(console_load_data())
    rows=[[f"Template {i+1} [{('ON' if t['enabled'] else 'OFF')}]" ] for i,t in enumerate(d['templates'])]
    rows.append(["Add Template"])
    if d['templates']: rows.append(["Delete Template"])
    rows.append(["Back"])
    return console_kb(rows)

def console_settings_kb():
    return console_kb([["Set Channel","Set Time"],["Set 10-Min Limit","Set Amount Rules"],["Set Breakage %","Set Complete %"],["ON/OFF"],["View Template"],["Send Test"],["Back"]])

async def console_open(m):
    if m.from_user.id != ADMIN_ID: return
    CONSOLE_STATE.pop(ADMIN_ID,None)
    return await ans(m, "Console Message Manager\n\n" + console_status_text(), reply_markup=console_main_kb())

async def console_send_test(m):
    if m.from_user.id != ADMIN_ID: return
    d=console_ensure_templates(console_load_data()); idx=CONSOLE_STATE.get(ADMIN_ID,{}).get('template_index')
    if idx is None or idx>=len(d['templates']): return await ans(m,"আগে একটি Template select করুন।",reply_markup=console_template_kb())
    t=d['templates'][idx]
    if not t.get('channel_id'): return await ans(m,"এই Template-এর Channel আগে সেট করুন।",reply_markup=console_settings_kb())
    try:
        await m.bot.send_message(chat_id=t['channel_id'], text=console_generate_template(t['text'],t))
...
async def console_action(m):
    if m.from_user.id != ADMIN_ID: return
    text=(m.text or '').strip(); st=CONSOLE_STATE.setdefault(ADMIN_ID,{})
    d=console_ensure_templates(console_load_data()); action=st.get('action')
    if text == "Back to Admin Panel":
        CONSOLE_STATE.pop(ADMIN_ID,None); return await ans(m,"Admin Panel",reply_markup=am())
    if text == "Back":
        st.clear(); return await ans(m,"Console Message Manager\n\n"+console_status_text(),reply_markup=console_main_kb())
    if text == "Console": return await console_open(m)
    if text == "Template Manager":
        st.pop('action',None); return await ans(m,f"Template Manager\n\nTotal Templates: {len(d['templates'])}",reply_markup=console_template_kb())
    if text == "Status": return await ans(m,console_status_text(),reply_markup=console_main_kb())
    if text == "Request ID":
        return await ans(m,f"Last used Request ID: #{d['request_id']}\nNext Request ID: #{int(d['request_id'])+1}\n\nRequest ID only increases when a template contains {{#3+updated}}.",reply_markup=console_main_kb())
    if text == "Add Template":
        st['action']='add_template'; return await ans(m,f"Template {len(d['templates'])+1}-এর সম্পূর্ণ multiline message পাঠান.\nপুরো message-টাই ১টি Template হবে.\n\nCancel")
    if text == "Delete Template":
        st['action']='delete_template'; return await ans(m,f"যে Template delete করবেন তার number দিন: 1-{len(d['templates'])}\n\nCancel")
    mm=re.fullmatch(r"Template (\d+) \[(ON|OFF)\]",text)
    if mm:
        idx=int(mm.group(1))-1
        if 0<=idx<len(d['templates']):
            st['template_index']=idx; st.pop('action',None); t=d['templates'][idx]
            return await ans(m,f"Template {idx+1}\n\n{t['text']}\n\nChannel: {t.get('channel_id') or 'Not set'}\nLimit: {t.get('min_per_10m',1)}-{t.get('max_per_10m',1)}\nTime: {t.get('interval_seconds',600)} sec\nBreakage: {t.get('breakage_probability',98)}%\nComplete: {t.get('complete_probability',2)}%\nAmount Rules: {console_format_amount_rules(t)}\nStatus: {'ON' if t.get('enabled') else 'OFF'}",reply_markup=console_settings_kb())
        return
    idx=st.get('template_index')
    if text in {"Set Channel","Set Time","Set 10-Min Limit","Set Amount Rules","Set Breakage %","Set Complete %","ON/OFF","View Template","Send Test"} and (idx is None or idx>=len(d['templates'])):
        return await ans(m,"আগে একটি Template select করুন।",reply_markup=console_template_kb())
    if text == "Send Test": return await console_send_test(m)
    if idx is not None and 0<=idx<len(d['templates']):
        t=d['templates'][idx]
        if text == "Set Channel": st['action']='channel'; return await ans(m,"এই Template-এর target channel ID পাঠান.\nExample: -1001234567890\n\nCancel")
        if text == "Set Time": st['action']='time'; return await ans(m,"এই Template কত second পরপর নতুন cycle শুরু করবে লিখুন.\nExample: 600 = 10 minutes\n\nCancel")
        if text == "Set 10-Min Limit": st['action']='limit'; return await ans(m,"এই Template-এর প্রতি cycle-এ message count range দিন.\nExample: 3-7\n\nCancel")
        if text == "Set Amount Rules": st['action']='amount_rules'; return await ans(m,"৪টি amount range ও percentage একসাথে দিন, প্রতি লাইনে এই format-এ:\n\n25-200 90%\n200-1000 7%\n1000-3900 2%\n3900-5000 1%\n\nসব percentage মিলে 100% হতে হবে.\nCancel")
        if text in ("Set Breakage %","Set Complete %"):
            st['action']='breakage_probability' if text=="Set Breakage %" else 'complete_probability'; return await ans(m,f"বর্তমান Breakage: {t.get('breakage_probability',98)}%\nবর্তমান Complete: {t.get('complete_probability',2)}%\n\nPercentage দিন (0-100). Breakage + Complete মিলে 100% হওয়া উচিত.\n\nCancel")
        if text == "ON/OFF":
            t['enabled']=not t.get('enabled',False); t['_next_run']=0 if t['enabled'] else None
            if not t['enabled']: t.pop('_next_run',None)
            d['templates'][idx]=t; console_save_data(d); return await ans(m,f"Template {idx+1} এখন {'ON' if t['enabled'] else 'OFF'}.",reply_markup=console_settings_kb())
        if text == "View Template": return await ans(m,t['text'],reply_markup=console_settings_kb())
    if action == 'add_template':
        if not text or text.lower()=='cancel': st.clear(); return await ans(m,"Cancelled.",reply_markup=console_template_kb())
        d['templates'].append(normalize_template(text,len(d['templates']))); console_save_data(d); st.clear(); return await ans(m,f"Template {len(d['templates'])} added successfully.",reply_markup=console_template_kb())
    if action == 'delete_template':
        if not text.isdigit(): return await ans(m,"শুধু Template number দিন।")
        i=int(text)-1
        if not 0<=i<len(d['templates']): return await ans(m,"Invalid Template number.")
        d['templates'].pop(i)
        for j,t in enumerate(d['templates']): t['name']=f"Template {j+1}"
        console_save_data(d); st.clear(); return await ans(m,"Template deleted.",reply_markup=console_template_kb())
    if action in ('breakage_probability','complete_probability'):
        if not text.isdigit() or not 0<=int(text)<=100: return await ans(m,"0-100 এর মধ্যে percentage দিন।")
        field=action; other='complete_probability' if field=='breakage_probability' else 'breakage_probability'; t[field]=int(text); d['templates'][idx]=t; console_save_data(d); total=int(t.get(other,0))+int(t[field])
        if total!=100: return await ans(m,f"Saved. Current total: {total}%. Breakage + Complete মিলে 100% করুন.")
        st.pop('action',None); return await ans(m,f"Template {idx+1} amount workflow saved.\n\nBreakage: {t[field if field=='breakage_probability' else other]}%\nComplete: {t[other if field=='breakage_probability' else field]}%",reply_markup=console_settings_kb())
    if action == 'amount_rules':
        ...
        for line in text.splitlines():
            ...
            if not mm: return await ans(m,"Format ভুল. ঠিক এইভাবে ৪টি line দিন:\n\n25-200 90%\n200-1000 7%\n1000-3900 2%\n3900-5000 1%")
            a,b,p=map(int,mm.groups())
            if a>b or not 0<=p<=100: return await ans(m,"Range ও percentage সঠিকভাবে দিন।")
            parsed.append({'min':a,'max':b,'probability':p})
        if len(parsed)!=4 or sum(x['probability'] for x in parsed)!=100: return await ans(m,"ঠিক ৪টি range দিন এবং percentage মোট 100% করুন।")
        t['amount_rules']=parsed; d['templates'][idx]=t; console_save_data(d); st.pop('action',None); return await ans(m,f"Template {idx+1} amount rules saved.\n\n{console_format_amount_rules(t)}",reply_markup=console_settings_kb())
    if action == 'channel':
        if not re.fullmatch(r'-?\d+',text): return await ans(m,"Valid Telegram channel ID দিন।")
        t['channel_id']=text; t['_next_run']=0 if t.get('enabled') else t.get('_next_run'); d['templates'][idx]=t; console_save_data(d); st.pop('action',None); return await ans(m,"Channel saved.",reply_markup=console_settings_kb())
    if action == 'time':
        if not text.isdigit() or int(text)<1: return await ans(m,"Positive seconds দিন।")
        t['interval_seconds']=int(text); t['_next_run']=0 if t.get('enabled') else t.get('_next_run'); d['templates'][idx]=t; console_save_data(d); st.pop('action',None); return await ans(m,"Template time saved.",reply_markup=console_settings_kb())
    if action == 'limit':
        mm=re.fullmatch(r'\s*(\d+)\s*[-:]\s*(\d+)\s*',text)
        if not mm: return await ans(m,"Format: 3-7")
        lo,hi=map(int,mm.groups())
        if hi<lo: return await ans(m,"সঠিক range দিন।")
        t['min_per_10m']=lo; t['max_per_10m']=hi; t['_next_run']=0 if t.get('enabled') else t.get('_next_run'); d['templates'][idx]=t; console_save_data(d); st.pop('action',None); return await ans(m,"Template message limit saved.",reply_markup=console_settings_kb())
    if action and action in ('channel','time','limit','amount_rules','breakage_probability','complete_probability'): return
...
async def console_scheduler(bot):
    while True:
        try:
            ...
            changed=False
            for i,t in enumerate(d['templates']):
                if not t.get('enabled') or not t.get('channel_id') or not t.get('text'): continue
                now=asyncio.get_running_loop().time(); due=t.get('_next_run')
                if due is not None:
                    try:
                        if now<float(due): continue
                    except (TypeError,ValueError): pass
                lo=max(0,int(t.get('min_per_10m',1))); hi=max(lo,int(t.get('max_per_10m',1))); count=random.randint(lo,hi)
                t['_next_run']=now+max(1,int(t.get('interval_seconds',600))); d['templates'][i]=t; changed=True
                for _ in range(count):
                    cur=console_ensure_templates(console_load_data())
                    if i>=len(cur['templates']): break
                    ct=cur['templates'][i]
                    if not ct.get('enabled') or not ct.get('channel_id'): break
                    try: await bot.send_message(chat_id=ct['channel_id'],text=console_generate_template(ct['text'],ct))
                    except Exception as e: print(f"Console template {i+1} send error: {e}")
                    await asyncio.sleep(random.uniform(0.5,2.0))
            if changed: console_save_data(d)
        except Exception as e: print('Console scheduler error:',e)
...
if not BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN is missing. Put your BotFather token in the BOT_TOKEN line at the top of this file.")
if not ADMIN_ID:
    raise RuntimeError("ADMIN_ID is missing. Put your Telegram admin user ID in the ADMIN_ID line at the top of this file.")
...
BUTTON_EMOJI_IDS = {
    ...
    "admin_console": "5341715473882955310",
...
# Console navigation/action handler. It is registered before the broad admin handler.
dp.message.register(
    console_action,
    F.from_user.id == ADMIN_ID,
    lambda m: (clean_button_text(m.text) in {"Console","Template Manager","Request ID","Status","Add Template","Delete Template","Back","Back to Admin Panel","Set Channel","Set Time","Set 10-Min Limit","Set Amount Rules","Set Breakage %","Set Complete %","ON/OFF","View Template","Send Test"} or ADMIN_ID in CONSOLE_STATE)
)
...
async def main():
    b=Bot(BOT_TOKEN,default=DefaultBotProperties(parse_mode=ParseMode.HTML))
