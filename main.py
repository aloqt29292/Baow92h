...
async def referral_section(m):
    if not await ensure_joined(m): return
    access=c.execute("SELECT * FROM referral_access WHERE user_id=?",(m.from_user.id,)).fetchone()
    ...
    bot_me=await m.bot.get_me(); link=referral_link(m.from_user.id,bot_me.username)
    # CopyTextButton is supported by modern Telegram clients. If unavailable in the installed aiogram,
    # the fallback opens the link directly instead of exposing it in message text.
    try:
        from aiogram.types import CopyTextButton
        link_btn=InlineKeyboardButton(text="🔗 Copy Referral Link", copy_text=CopyTextButton(text=link))
    except Exception:
        link_btn=InlineKeyboardButton(text="🔗 Open Referral Link", url=link)
    kb=InlineKeyboardMarkup(inline_keyboard=[[link_btn],[InlineKeyboardButton(text="📊 Referral Statistics",callback_data="ref_stats")]])
    await ans(m,"Referral Center\n\nনিচের বাটন থেকে আপনার Referral Link copy করুন অথবা Referral Statistics দেখুন।",reply_markup=kb)
...
dp.message.register(referral_section,F.text, lambda m: clean_button_text(m.text)=="Referral")
